...
export default function MobileHome({locale}:{locale:Locale}){
 ...
 return <main className="mobile-home" id="main-content" ref={root}>
  ...
  {chapters.map(([kind,slug],i)=><section className={`mobile-chapter chapter-${kind}`} key={kind} id={kind}><MobileStory kind={kind} locale={locale} chapter={`0${i+1}`} /><Link className="mobile-link" href={`/${locale}/projects/${slug}${kind==='studio'?'#studio':''}`}>{t('Explore the full project','了解完整项目')}<ArrowIcon direction="up-right"/></Link></section>)}
