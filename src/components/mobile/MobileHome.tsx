...
export default function MobileHome({locale}:{locale:Locale}){
 ...
 return <main className="mobile-home" id="main-content" ref={root}>
  ...
  {chapters.map(([kind,slug],i)=><section className={`mobile-chapter chapter-${kind}`} key={kind} id={kind}><div className="mobile-chapter-intro"><p className="mobile-eyebrow">0{i+1} / {kind==='studio'?'INNERFY STUDIO':pick(pageCopy[slug].eyebrow,locale).replace(/^\d+ \/ /,'')}</p><h2>{kind==='studio'?t('From theory to a working tool.','从理论到可用工具。'):pick(pageCopy[slug].title,locale)}</h2><p>{kind==='studio'?t('Inspect, evaluate, and approve through a local desktop workflow.','通过本地桌面流程检查、评估与审批。'):kind==='rental'?t('Discover a neighborhood, compare practical details, and keep a shortlist.','发现区域、比较实际条件并建立候选清单。'):pick(pageCopy[slug].intro,locale)}</p></div><MobileStory kind={kind} locale={locale}/><Link className="mobile-link" href={`/${locale}/projects/${slug}${kind==='studio'?'#studio':''}`}>{t('Explore the full project','了解完整项目')}<ArrowIcon direction="up-right"/></Link></section>)}
