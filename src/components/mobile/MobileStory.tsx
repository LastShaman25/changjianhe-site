...
import './mobile.css';
export default function MobileStory({kind,locale,chapter}:{kind:StoryKind;locale:Locale;chapter?:string}){
 ...
 return <><div className={`mobile-story mobile-story-${kind}`} ref={track} style={{'--story-travel':`${story.steps.length*distance}px`} as CSSProperties} data-story={kind} data-step={step} data-progress={progress.toFixed(3)}>
  ...
  <div className="mobile-story-card" ref={card}>
   <div className="mobile-story-top">{chapter?<h2><small>{chapter} / </small>{pick(story.name,locale)}</h2>:<span>{pick(story.name,locale)}</span>}<span>0{step+1} / 0{story.steps.length}</span></div>
